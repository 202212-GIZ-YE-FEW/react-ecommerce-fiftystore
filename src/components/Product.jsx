...
import fillingStars from "../../utils/fillingStars"
                    ...
                    <div className="flex justify-center items-center mt-2.5 mb-5">
                        {[1, 2, 3, 4, 5].map((i)=> {
                            if(i >= Math.round(rate))
                             return <StarIcon className="w-5 h-5 text-yellow-300" fill="currentColor"/>
                            else
                             return <StarIcon className="w-5 h-5 text-stone-600"/>
                        })}
